...
class DealModel extends DBConnection{
    protected function getDeals() {
        $sql = "SELECT m.id as meal_id, m.food_name, m.food_image, r.restaurant_name, r.deal_id,
         ...
         e.name
         FROM meals AS m
        LEFT JOIN restaurants AS r ON m.id = r.deal_id
    ...
    protected function getReviews($id) {
        $sql = "SELECT re.review, re.name, r.id
         FROM review AS re
        LEFT JOIN restaurants AS r ON r.id = re.restaurant_id
        WHERE r.id = ?";
        $this->prepare($sql);
        $this->bindValue(1, $id);
        $this->execute();
